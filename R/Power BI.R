...
platformAVG <- allPosts %>% 
  group_by(post_type) %>%
  dplyr::summarize(
    impressionsAVG = round(mean(impressions), 1),
    engagementsAVG = round(mean(engagements), 1),
    engrtAVG = round(mean(engagementRate), 3),
    postclicksAVG = round(mean(postClicks), 1),
    sharesAVG = round(mean(shares), 1))

taggedPosts <- taggedPosts %>% 
  left_join(platformAVG) %>% 
  mutate(iVA = round((impressions - impressionsAVG)/impressionsAVG, 3),
         eVA = round((engagements - engagementsAVG)/engagementsAVG, 3),
         erVA = round((engagementRate - engrtAVG)/engrtAVG, 3),
         pcVA = round((postClicks - postclicksAVG)/postclicksAVG, 3),
         sVA = round((shares - sharesAVG)/sharesAVG), 3) 

# push data
print('push social media data')

write_sheet(taggedPosts, ss = ss, sheet = 'Social - Campaign')
...
token4 <- ## INSERT TOKEN HERE
token5 <- ## INSERT TOKEN HERE
businessID <- ## INSERT BUSINESS ID HERE
